(() => {
  ...
  function renderOverview(report) {
    ...
    $("healthTimestamp").textContent = report.timestamp ? `Updated ${formatTime(report.timestamp)}` : "—";

    const config = report.configuration || report.config || {};
    $("envValue").textContent = firstDefined(config.environment, config.nodeEnv, report.environment, "—");
  ...
  function renderMetrics(data) {
    ...
    const cpu = Number(firstDefined(
      data.cpuUsage, data.cpu, data.process?.cpu, data.runtime?.cpu
    ));
    const memory = Number(firstDefined(
      data.memoryUsage, data.memory, data.system?.memoryUsage, data.runtime?.memory
    ));
    ...
    $("loadValue").textContent = Array.isArray(load) ? load.map(v => Number(v).toFixed(2)).join(" / ") : (load ?? "—");
    ...
    $("messagesValue").textContent = firstDefined(data.messagesProcessed, data.messages, data.runtime?.messages, 0);
    $("commandsValue").textContent = `${firstDefined(data.commandsExecuted, data.commands, data.runtime?.commands, 0)} cmds`;
  ...
  function renderRuntime(data) {
    ...
    $("runtimePanel").innerHTML = `<div class="panel-header"><div><span class="eyebrow">NODE PROCESS</span><h2>Runtime</h2></div></div>
    ...
    $("configPanel").innerHTML = `<div class="panel-header"><div><span class="eyebrow">CONFIGURATION</span><h2>Public snapshot</h2></div></div>
  ...
  function renderApis(list) {
    ...
    $("apiGrid").innerHTML = providers.length ? providers.map(p => {
    ...
    }).join("") : `<div class="empty">No API provider data.</div>`;
  }
  ...
  function renderNotifications(list) {
    ...
    $("notificationList").innerHTML = items.slice(0, 30).map(item => {
      const type = String(item.type || item.event || "info").toLowerCase();
      const icon = type.includes("error") ? "✕" : type.includes("reconnect") ? "↻" : type.includes("login") ? "↪" : "●";
  ...
  async function refresh() {
    ...
    try {
      ...
      $("lastUpdate").textContent = `Updated ${formatTime(new Date())} · ${Math.round(performance.now() - started)}ms`;
      ...
      $("heroMessage").textContent = "Cannot reach the dashboard API. The UI will retry automatically.";
