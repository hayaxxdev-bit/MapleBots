...
Maple Bots is currently a WhatsApp-first modular monolith. Telegram is intentionally deferred.

```text
src/
├── app/                    # startup, lifecycle, shutdown, application events
├── platforms/whatsapp/     # Baileys connection and WhatsApp message handling
├── commands/               # bot commands (kept unchanged during this refactor)
├── services/               # application/business services
├── infrastructure/         # database, cache, HTTP, scraper, browser, logging, metrics
├── web/dashboard/          # dashboard and health endpoints
├── config/                 # validated environment configuration
├── types/                  # shared TypeScript types
└── utils/                  # small generic helpers only
```

The old `core/`, `connection/`, `handlers/`, `dashboard/`, `scrapers/`, and infrastructure-like `utils/*` paths are kept as temporary compatibility re-exports so existing command imports continue to work while the command layer is migrated separately.

Runtime state belongs under `data/` and must not be committed:
