...
export class ScraperManager {
      ...
      for (const scraper of scrapers) {
        ...
        if (health.status === 'down') {
          ...
          if (config.scraperAutoSwitch) {
            await this.switchToFallback(type);
          }
        }
      }
    }
  ...
  private async switchToFallback(type: string): Promise<void> {
    const scrapers = this.scrapers.get(type);
    if (!scrapers || scrapers.length === 0) return;
  ...
  async execute<T>(
    ...
    if (!scrapers || scrapers.length === 0) {
      ...
      } as ScraperResult<T>; // 👈 1. Tambahkan type assertion di sini
    }
    ...
    if (activeScraper) {
      try {
        ...
        if (result.success) {
          return result as ScraperResult<T>; // 👈 2. Cast result ke ScraperResult<T>
        }
      } catch (error) {
    ...
    } as ScraperResult<T>; // 👈 4. Tambahkan type assertion di sini
  }
