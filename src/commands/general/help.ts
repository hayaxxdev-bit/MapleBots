...
function createHelpMessage(): string {
  const downloaderCommands = commands.filter(
    (cmd: { category: string }) => cmd.category === 'downloader'
  );
  const animeCommands = commands.filter((cmd: { category: string }) => cmd.category === 'anime');
  const generalCommands = commands.filter(
    (cmd: { category: string }) => cmd.category === 'general'
  );
  ...
  handler: async (ctx: CommandContext): Promise<void> => {
    ...
    if (args.length > 0) {
      ...
      const command = getAllCommands().find(
        (cmd) => cmd.name.toLowerCase() === commandName || cmd.aliases?.includes(commandName ?? '')
      );
