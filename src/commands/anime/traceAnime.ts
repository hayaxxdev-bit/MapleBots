...
function getQuotedMessage(msg: proto.IWebMessageInfo): proto.IWebMessageInfo | null {
  ...
  if (!ctx?.quotedMessage || !msg.key?.remoteJid) {
    return null;
  }
...
async function traceAnime(imageBuffer: Buffer): Promise<TraceMoeResult | null> {
  const res = await http.post<TraceMoeApiResponse>(config.traceMoeUrl, imageBuffer, {
    headers: { 'Content-Type': 'image/jpeg' },
  });

  const responseData = res as unknown as HttpResponse<TraceMoeApiResponse>;
...
export const traceAnimeCommand: Command = {
  ...
  handler: async (ctx) => {
    ...
    const targetMsg = quoted?.message?.imageMessage
      ? quoted
    ...
    try {
      ...
      const result = await traceAnime(buffer);

      if (!result) {
        await ctx.reply(
          '❌ Anime tidak ditemukan dari gambar ini. Coba screenshot yang lebih jelas.'
        );
        return;
      }
