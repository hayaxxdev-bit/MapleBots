...
async function getRandomWallpaper(category: string): Promise<string> {
  const res = await http.get<WaifuPicsResponse>(`https://api.waifu.pics/sfw/${category}`);
  return res.data.url;
}
...
export const wallpaperCommand: Command = {
  ...
  handler: async (ctx) => {
    ...
    if (!ALLOWED_CATEGORIES.includes(category)) {
      await ctx.reply(`⚠️ Kategori tidak dikenal. Pilih salah satu: ${ALLOWED_CATEGORIES.join(', ')}`);
      return;
    }
