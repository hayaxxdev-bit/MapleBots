 ...
 */
/**
 * Download video pakai yt-dlp, target tetap 720p.
 * Fallback turun ke kualitas lebih rendah HANYA kalau 720p ternyata
 * melebihi batas ukuran WhatsApp (video panjang/bitrate tinggi).
 */
async function downloadVideoWithYtDlp(url: string): Promise<YouTubeDownloadResult> {
  ...
  const QUALITY_TARGET = [720, 480, 360]; // 720p tetap, turun cuma kalau kebesaran
  let lastError: Error | null = null;

  for (const height of QUALITY_TARGET) {
    ...
    try {
      ...
      if (!filePath) throw new Error('yt-dlp tidak menghasilkan file output');

      const stat = await fs.stat(filePath).catch(() => null);
      if (!stat) throw new Error('File hasil yt-dlp tidak ditemukan');
      ...
      lastError = new Error(err?.stderr || err?.message || 'yt-dlp gagal');
      await cleanupTempFiles(tmpDir, id);
    }
  }
  ...
  try {
    const { stdout } = await execFileAsync(
      ...
      [
        ...
        '-f', 'bestaudio',
        '-x', '--audio-format', 'mp3',
        '--audio-quality', '0', // kualitas MP3 terbaik
        '-o', outputTemplate,
        '--no-playlist',
        '--no-warnings',
        '--no-cache-dir',
    ...
    await cleanupTempFiles(tmpDir, id);
    throw new Error(err?.stderr || err?.message || 'yt-dlp gagal');
  }
}
...
async function getTitleSafe(url: string): Promise<string> {
  try {
    const { stdout } = await execFileAsync('yt-dlp', [
      url, '--print', 'title', '--no-warnings', '--no-playlist', '--no-cache-dir',
    ]);
...
async function downloadYouTubeVideo(url: string): Promise<YouTubeDownloadResult> {
  try {
    ...
    if (!downloadUrl) throw new Error('Gagal mendapatkan link download dari API');
...
async function getVideoInfo(url: string): Promise<YouTubeVideoInfo | null> {
  try {
    ...
    return {
      ...
      likes: info.like_count ? StringHelper.formatNumber(info.like_count) : 'Hidden',
...
export const youtubeCommand: Command = {
  ...
  handler: async (ctx) => {
    ...
    try {
      ...
      logHelper.warn('youtube-cmd', `Video download failed: ${error?.message || error}`);
      if (error?.message?.includes('maxContentLength') || error?.message?.includes('exceeded')) {
        await ctx.reply('❌ Gagal: Ukuran video terlalu besar untuk dikirim ke WhatsApp (>50MB) bahkan di kualitas terendah.');
      } else {
...
export const youtubeAudioCommand: Command = {
  ...
  handler: async (ctx) => {
    ...
    try {
      ...
      logHelper.warn('youtube-cmd', `Audio download failed: ${error?.message || error}`);
      if (error?.message?.includes('maxContentLength') || error?.message?.includes('exceeded')) {
...
export const youtubeShortsCommand: Command = {
  ...
  handler: async (ctx) => {
    ...
    try {
      ...
      logHelper.warn('youtube-cmd', `Shorts download failed: ${error?.message || error}`);
      if (error?.message?.includes('maxContentLength') || error?.message?.includes('exceeded')) {
